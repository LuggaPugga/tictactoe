...
const LocalBoard: React.FC<LocalBoardProps> = ({
  ...
  const getCellStyle = (value: CellValue) => {
    let style = "w-full h-full flex items-center justify-center transition-colors duration-200 p-2 "
  ...
  return (
    <div className="aspect-square overflow-hidden relative">
          ...
          {board.map((cell, cellIndex) => (
            ...
            >
              {cell && !winner ? (
                ...
                >
                  {cell === "X" ? <X className="w-12 h-12" /> : <Circle className="w-10 h-10" />}
                </motion.div>
              ) : (
      ...
      {winner && (
        ...
        >
          {winner === "X" ? <X className="w-4/5 h-4/5" /> : <Circle className="w-4/6 h-4/6" />}
        </div>
      )}
    </div>
  )
